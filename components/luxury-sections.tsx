...
export function MobileBuyBar({ onBuy, product }:{ onBuy:()=>void; product:Product }) {
  return <div className="fixed inset-x-0 bottom-0 z-40 border-t border-black/10 bg-white/95 p-3 pb-[max(.75rem,env(safe-area-inset-bottom))] shadow-[0_-12px_40px_rgba(0,0,0,.12)] backdrop-blur-xl md:hidden dark:border-white/10 dark:bg-neutral-950/95"><div className="mx-auto flex max-w-md items-center gap-3"><div className="min-w-0 flex-1"><p className="truncate text-[11px] text-neutral-500">{product.name}</p><b className="text-base">{money(product.price)}</b></div><a href={whatsappUrl} target="_blank" rel="noreferrer" aria-label="WhatsApp-аар захиалах" className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[#25D366] text-white"><MessageCircle size={20}/></a><button onClick={onBuy} className="flex h-12 items-center gap-2 rounded-full bg-ink px-5 text-sm font-bold text-white dark:bg-white dark:text-ink"><ShoppingBag size={17}/> Худалдаж авах</button></div></div>;
}
