...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  ...
  return {title:product.name,description:product.description,openGraph:{title:`${product.name} | TOMOC Store`,description:product.benefit,images:[{url:product.image,alt:product.name}]},alternates:{canonical:`/products/${product.id}`}};
}
...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
  ...
  const schema={"@context":"https://schema.org","@type":"Product",name:product.name,image:[product.image],description:product.description,brand:{"@type":"Brand",name:"TOMOC Store"},offers:{"@type":"Offer",priceCurrency:"MNT",price:product.price,availability:"https://schema.org/InStock",url:`${process.env.NEXT_PUBLIC_SITE_URL||"http://localhost:3000"}/products/${product.id}`}};
  return <main className="min-h-screen bg-[var(--bg)]">
    ...
    <section className="container grid gap-10 py-8 md:grid-cols-[1.06fr_.94fr] md:items-start md:py-16">
      <div className="relative aspect-[4/4.8] overflow-hidden rounded-[34px] bg-cloud"><Image src={product.image} alt={product.name} fill priority sizes="(max-width:768px) 100vw, 55vw" className="object-cover"/>{product.badge&&<span className="absolute left-5 top-5 rounded-full bg-white px-4 py-2 text-xs font-bold text-ink shadow">{product.badge}</span>}</div>
      <div className="md:sticky md:top-28"><span className="eyebrow">{product.category}</span><h1 className="display-serif mt-4 text-[48px] leading-[.95] tracking-[-.045em] md:text-[66px]">{product.name}</h1><p className="mt-6 text-[17px] leading-7 text-neutral-500">{product.description}</p><div className="mt-6 text-3xl font-bold">{money(product.price)}</div><div className="my-7 grid grid-cols-3 gap-2 text-center text-[11px] font-bold"><div className="card p-3"><Truck className="mx-auto mb-2" size={19}/>24–48 цаг</div><div className="card p-3"><ShieldCheck className="mx-auto mb-2" size={19}/>Баталгаатай</div><div className="card p-3"><Check className="mx-auto mb-2" size={19}/>Бэлэн бараа</div></div><ProductActions product={product}/><div className="mt-8 border-t border-black/10 pt-7 dark:border-white/10"><h2 className="flex items-center gap-2 font-bold"><Sparkles size={18} className="text-gold"/> Гол давуу тал</h2><p className="mt-3 leading-7 text-neutral-500">{product.benefit}</p></div></div>
    </section>
